...
/**
 * BlockOrbit — one bright cell circling the rim of a drawn grid without
 * stopping, trailed by two dimmer cells in lockstep.
 */
function BlockOrbit({
  className,
  size = 5,
  ...
  return (
    <span
      ...
      {...props}
    >
      <style>{`@keyframes gl-block-orbit {
          ...
          style={
            {
              '--gl-x': `${cells - 1}ch`,
              '--gl-y': `${cells - 1}ch`,
              animation: 'gl-block-orbit var(--duration, 3s) linear infinite',
